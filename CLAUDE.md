...
- **Vault tools** (19): `search_vault`, `read_note` (supports batch via `paths` array), `list_notes`, `get_links`, `explore_structure`, `list_tags`, `get_manual_context`, `get_properties`, `get_file_info`, `find_dead_links`, `query_notes`, `get_vault_stats`, `get_chat_history`, `get_note_stats`, `get_note_connections`, `get_selection`, `preview_pending_edits`, `find_orphan_notes`, `find_unlinked_mentions`
